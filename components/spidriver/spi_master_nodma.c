...
esp_err_t spi_device_transmit(spi_nodma_device_handle_t handle, spi_nodma_transaction_t *trans_desc,  TickType_t ticks_to_wait)
{
    esp_err_t ret;
    spi_nodma_transaction_t *ret_trans;
    //ToDo: check if any spi transfers in flight
    ret=spi_device_queue_trans(handle, trans_desc, ticks_to_wait);
    if (ret!=ESP_OK) return ret;
    ret=spi_device_get_trans_result(handle, &ret_trans, ticks_to_wait);
    if (ret!=ESP_OK) return ret;
